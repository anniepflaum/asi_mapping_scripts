...
def plot_pretty():
    ...
    # FAST
    fig = plt.figure(figsize=(15, 10))
#    gs = gridspec.GridSpec(4,2, width_ratios=[5,1])
    gs = gridspec.GridSpec(4,4, width_ratios=[4,0.2,0.2,1])
    ax = fig.add_subplot(gs[:,0])
...
#    try:
    ...
    # FAST
    ax.plot(lon1, lat1, color='red', label='GNEISS trajectory',zorder=7)#, transform=ccrs.PlateCarree(), zorder=7)
    ax.scatter(lonm1, latm1, color='red', s=15, zorder=7)#, transform=ccrs.PlateCarree(), zorder=7)
    ax.scatter(lona1, lata1, color='magenta', marker='x', zorder=7)#, transform=ccrs.PlateCarree(), zorder=8)
    ax.plot(lon2, lat2, color='red', zorder=7)#, transform=ccrs.PlateCarree(), zorder=7)
    ax.scatter(lonm2, latm2, color='red', s=15, zorder=7)#, transform=ccrs.PlateCarree(), zorder=7)
    ax.scatter(lona2, lata2, color='magenta', marker='x', zorder=7)#, transform=ccrs.PlateCarree(), zorder=8)
    ax.legend(loc='upper right')
...
#        #############################
        ...
        # FAST
        im_handle = ax.pcolor(skymaps[site]['lon'], skymaps[site]['lat'], im)#, transform=ccrs.PlateCarree())
        ax1[site].pcolor(skymaps[site]['lon'], skymaps[site]['lat'], img)#, transform=ccrs.PlateCarree())
        ax1[site].set_title(site)
        ax.plot(skymaps[site]['site_lon'],skymaps[site]['site_lat'],color='m')

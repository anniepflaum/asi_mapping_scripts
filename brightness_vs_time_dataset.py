...
from map_asi_archive import (
    ...
    get_site_tiff_candidates,
    load_best_frame_from_tiffs,
    load_skymaps,
    load_traj,
    parse_date_and_time,
    ...
    while t <= end_dt:
        ...
        for site in ["ARV", "VEE", "BVR"]:
            if site not in selected_sites:
                continue
            try:
                im_raw, _ = load_best_frame_from_tiffs(
                    site,
                    tiff_candidates.get(site, []),
                    t,
        ...
        _, _, _, _, _, _, lat_l, lon_l = load_traj("Traj_Left.txt", map_time=time_arg)
        _, _, _, _, _, _, lat_r, lon_r = load_traj("Traj_Right.txt", map_time=time_arg)
